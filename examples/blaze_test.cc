...
T test(size_t size, size_t seed, double& time1, double& time2, double& time3, double& time4)
{
  ...
  typedef blaze::DynamicVector<T>         Vector;
  typedef blaze::StaticVector<T, DOW>     WorldVector;
  
  typedef blaze::DynamicMatrix<T>  Matrix;
  typedef blaze::HybridMatrix<T, DOW, DOW>         WorldMatrix;
  ...
  for (size_t i = 0; i < size; ++i) {
    nrm1[i] = std::sqrt((points1[i],points1[i]));
    nrm2[i] = (points2[i],points2[i]);
  }
